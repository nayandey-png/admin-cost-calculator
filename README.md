...
```
npm test      # 40 node:test unit tests against the calculation engine
...
src/ui.js         rendering, events, tooltips, CSV, printable report
...
The total is whatever the staffing choices come to, uncapped, and is reported as **cash
saving from your staffing changes with Effra**. `capacityValue` still appears separately,
as the value of the admin time Effra releases, but it no longer limits the total: a clinic
that cuts more hours than Effra frees up is making a bigger staffing decision, and the
wording says so rather than silently trimming the figure.
...
- **Batch versus per-invoice work.** Reading remittances and reconciling the bank are
  ...
  per-insurer CSV breakdown, and the last insurer takes the remainder so the split sums
...
- **Released capacity versus cash saving.** The two are reported as separate figures and
  never added together: `capacityValue` is what the freed admin time is worth, and the
  cash total is what the user's own staffing decision saves. Naming the cash total after
  the staffing change keeps them from reading as one number counted twice.
...
iframe without one inherits the parent page's origin, which is what keeps Export CSV and
Save results as PDF working. A `message` listener accepts height updates only
...
2319px on the results, 4486px with the detailed panel open, and back to 2319px when it is
...
**File size.** `PMICalculator.tsx` is **148 kB over 866 lines**, of which about 145 kB is
the base64 payload. That payload is split across roughly 800 lines of 180 characters
...
dialogue, and `allow-popups` so **Save results as PDF** can open the report in a new tab.
Without `allow-popups` the report still prints, from a hidden frame in place.
...
- [ ] Cash saving: keep gives 0, reduce is pro rata, remove gives the full employment
      cost, and the total is the uncapped sum of every role's choice.
- [ ] Aged debt: a typed amount drives the calculation and is split by revenue share; a
      typed zero counts as an answer; "Not sure" clears it and restores the estimate.
...
- [ ] Every question shows the same 16px gap above its Back / Not sure / Next row.

### Save results as PDF

- [ ] Opens a new tab holding the report and raises the print dialog, so the user can
      choose "Save as PDF".
- [ ] Works the same inside a srcdoc iframe, which is how Framer embeds it.
- [ ] With `window.open` blocked, the report prints from a hidden frame in place instead,
      and that frame is removed afterwards.
- [ ] The report carries the survey answers, the roles and salaries, the current process,
      the Effra comparison, the released-time choices, the Assumptions text and the
      disclaimer.
- [ ] The report is legible in pure greyscale and makes no network requests of its own.
...
- [ ] Framer component in a browser: the srcdoc frame reports `window.parent !== window`,
      inherits the page origin, auto-heights, and Export CSV and Save results as PDF
      still work.
